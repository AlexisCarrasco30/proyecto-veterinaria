          ...
          eventClick: function(info) {
              ...
              /*     info.el.style.borderColor = 'red'; */
                  ...
                  let fechaFormato; 
                    ...
                    auxHora = info.event.start.getHours()+":"+"0"+info.event.start.getMinutes(); 
                    }else{

                      auxHora =info.event.start.getHours()+":"+info.event.start.getMinutes(); 
                    }
             ...
             /*      tipoHora.innerHTML=`<input type="radio" id="${info.event.id}" name="horario" class="mp-2" value='${info.event.start.getHours()+":"+info.event.start.getMinutes()}'>Usted ha
                  ...
                  tituloTipo.innerHTML= `${info.event.title}`; 
                    ...
                    fechaFormato = info.event.start.getDate()+' /'+(info.event.start.getMonth()+1)+' /'+info.event.start.getFullYear()

                    tipoHora.innerHTML=`<input type="hidden" id="${info.event.id}" name="horario"  class="horario mp-2" value='${info.event.start.getHours()+":"+info.event.start.getMinutes()}'>
                    <p> Usted ha
                    seleccionado el turno del día <span class="text-info"> ${info.event.start.getDate()+' /'+(info.event.start.getMonth()+1)+' /'+info.event.start.getFullYear()}</span> a las <br> <span class="text-info">${auxHora}</span> <br> <strong>¿Confirma los datos? </strong>
                    </p>
                    ` 
                    tituloTipo.innerHTML= `${info.event.title}`; 
